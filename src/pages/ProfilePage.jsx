...
const ProfilePage = () => (
  <div className="max-w-xl mx-auto py-12 px-4">
    <div className="flex flex-col items-center bg-white rounded-lg shadow p-8">
      <img src={user.avatar} alt={user.name} className="w-24 h-24 rounded-full mb-4 border-4 border-tiffany-blue object-cover" />
      <h1 className="text-2xl font-bold mb-2">{user.name}</h1>
      <div className="text-gray-600 mb-1">{user.email}</div>
      <div className="text-gray-600 mb-1">{user.phone}</div>
      <div className="text-gray-500 text-sm mb-4">{user.city}, {user.address}</div>
